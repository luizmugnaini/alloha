...
uintptr_t align_forward(uintptr_t ptr, size_t alignment) {
    if (!is_power_of_two(alignment)) {
        fprintf(stderr, "align_forward expected a power of two alignment, got %zu.", alignment);
        return 0;
    }
...
void* arena_alloc_aligned(ArenaAlloc* arena, size_t size, size_t alignment) {
    ...
    fprintf(
        stderr,
        "Unable to allocate %zu bytes of memory, only %zu bytes left.",
        size,
...
void* arena_resize(
    ...
    if (new_size == old_mem_size) {
        fprintf(stderr, "Irrelevant resizing of same size %zu", old_mem_size);
        return old_mem;
    }
    if (!is_power_of_two(alignment)) {
        fprintf(stderr, "Expected the alignment to be a power of two, instead got %zu", alignment);
        return 0;
    }
    if (!old_mem) {
        fprintf(
